#!/usr/bin/env python3
"""Isolated GPU2 inference and evaluation of v2 Strong availability settings (nine additions plus CLR check).
...
EXP = ROOT / 'logs/exp_260806_000825_DecControlledASFStrong_A2FUSION_rlc_l1d256_l2p2t32d256g_l2g_scl_mha16'
CONFIG = EXP / 'config.yml'
CKPT = EXP / 'models/model_10.pt'
CASES = {'r': 'r', 'l': 'l', 'c': 'c', 'c_star': 'c', 'lr': 'lr',
         'cr': 'cr', 'cl': 'cl', 'clr': 'clr', 'c_star_lr': 'clr', 'cl_star_r': 'clr'}
...
REFERENCE = EXP / 'test_kitti/none/0.3/all'
...
def prepare(out):
    ...
    pipe.network.eval()
    assert len(pipe.dataset_test) == 13727, len(pipe.dataset_test)
    assert pipe.cfg.DATASET.label_version == 'v2_0'
    assert list(pipe.cfg.DATASET.roi.xyz) == [0., -16., -2., 72., 16., 7.6]
    assert pipe.is_validation_updated
    assert pipe.cfg.MODEL.FUSER.NAME == 'DecControlledA2Fusion'
    pipe.dict_cls_id_to_name = {1: 'Sedan', 2: 'Bus or Truck'}
...
def evaluate(out, pack, revised, smoke=False):
    from utils.kitti_eval.eval_revised import get_official_eval_result_revised as evaluator
    assert revised
    with gzip.open(pack, 'rt') as stream:
        records = [json.loads(line) for line in stream]
    assert len(records) == (3 if smoke else 13727)
    ...
    for case in CASES:
        ...
        for conf in ([0.3] if smoke else [0.3, 0.0]):
            ...
            for weather, ids in groups.items():
                ...
                # Revised evaluator defaults to 10 nonempty partitions. Smoke
                # repeats only its three inputs; never use smoke AP in a paper.
                eval_ids = ids * ((10 + len(ids) - 1) // len(ids)) if smoke else ids
                group = dict(frames=len(ids), classes={})
                for cls, name in enumerate(['sed', 'bus']):
                    metrics, report = evaluator([gt[i] for i in eval_ids], [dt[i] for i in eval_ids], cls,
                                                is_return_with_dict=True)
                    count = sum(int((gt[i]['name'] == name).sum()) for i in ids)
                    group['classes'][name] = dict(gt_count=count, defined=count > 0,
                        iou=[float(x) for x in metrics['iou']], BEV=[float(x) for x in metrics['bev']],
                        AP3D=[float(x) for x in metrics['3d']])
                    with (out / 'metrics.log').open('a') as stream:
                        stream.write(f'\n{case} conf={conf} condition={weather} class={name}\n{report}\n')
                group['mean'] = {k: [sum(group['classes'][c][k][j] for c in ['sed', 'bus']) / 2
                                     for j in range(3)] for k in ['AP3D', 'BEV']} if all(
                    group['classes'][c]['defined'] for c in ['sed', 'bus']) else None
                if smoke:
                    group.update(synthetic_smoke_only=True, repeated_evaluation_frames=len(eval_ids))
                results[case][str(conf)][weather] = group
                write(out / 'results_partial.json', results)
                print(f'[{now()}] RESULT {case} conf={conf} {weather}: {group}', flush=True)
    write(out / 'results.json', results)
    scope = ('SYNTHETIC SMOKE ONLY; three frames repeated to satisfy evaluator partitions.' if smoke else
             'Strong 08-06 model_10; all 13,727 v2 test frames; revised evaluator. Same weights for every setting.')
    lines = ['# Strong v2 availability: nine additional settings and CLR verification', '', scope, '',
             ...
             '|---|---:|---:|---:|---:|---:|---:|']
    for case in CASES:
        d = results[case]['0.3']['all']['classes']
        values = []
        for threshold in [0.3, 0.5]:
            x = [d[c]['AP3D'][d[c]['iou'].index(threshold)] for c in ['sed', 'bus']]
            values.extend(x + [sum(x) / 2])
        lines.append('| ' + case + ' | ' + ' | '.join(f'{x:.2f}' for x in values) + ' |')
    if not smoke:
        archive = json.loads(ARCHIVE.read_text())['0.3']
        deltas = {}
        for group in groups:
            deltas[group] = {}
            for cls in ['sed', 'bus']:
                d = results['clr']['0.3'][group]['classes'][cls]
                deltas[group][cls] = {kind: {f'{iou:g}': value - archive[group][cls][old][f'{iou:g}']
                    for iou, value in zip(d['iou'], d[kind])} for kind, old in [('AP3D', '3D'), ('BEV', 'BEV')]}
        max_delta = max(abs(v) for g in deltas.values() for c in g.values() for k in c.values() for v in k.values())
        write(out / 'clr_archive_check.json', dict(max_absolute_ap_delta=max_delta, deltas=deltas,
              note='New CLR results are retained even if archive differs; investigate any material drift.'))
        lines += ['', f'CLR archive verification: max absolute AP difference across all/weather, two classes and six metrics = {max_delta:.6f} points.']
        if max_delta > 0.05:
            lines += ['CHECK REQUIRED: CLR differs from archive by >0.05 AP points; do not silently combine old and new rows.']
        # The main paper-style table mirrors ASF Table 3; full weather tables
        # remain available for every class/metric/IoU. No-GT cells are N/A.
        for kind, threshold, conditions, title in [
                ('AP3D', 0.3, ['all', 'normal', 'overcast', 'sleet', 'heavysnow'], 'ASF Table 3 layout'),
                *[(k, t, list(groups), f'Full weather {k}@{t:g}') for k in ['AP3D', 'BEV'] for t in [0.3, 0.5, 0.7]]]:
            lines += ['', '## ' + title, '', '| Class | Setting | ' + ' | '.join(conditions) + ' |',
                      '|---|---|' + '|'.join(['---:'] * len(conditions)) + '|']
            for cls in ['sed', 'bus']:
                for case in CASES:
                    vals = []
                    for group in conditions:
                        d = results[case]['0.3'][group]['classes'][cls]
                        vals.append(f"{d[kind][d['iou'].index(threshold)]:.2f}" if d['defined'] else 'N/A (0 GT)')
                    lines.append('| ' + cls + ' | ' + case + ' | ' + ' | '.join(vals) + ' |')
...
def main():
    ...
    try:
        if args.evaluate_only:
            ...
            status(out, phase='initializing', started_at=now(), frames=0, total_frames=(3 if args.smoke else 13727),
            ...
            manifest = dict(created_at=now(), checkpoint=str(CKPT), checkpoint_sha256=digest(CKPT),
                            ...
                            memory_fraction=0.30, confs=[0.3, 0.0], detector_score_prefilter=0.1, nms_threshold=0.01,
                            selected_variant='Strong 08-06 model_10; best among archived full Strong runs; no new checkpoint search.',
                            variant_boundary='DecControlledA2Fusion: no task-context branch.',
                            protocol_reference='https://arxiv.org/html/2503.07029v2#S4.T3',
            ...
            indices = [0, 5000, 13726] if args.smoke else range(len(pipe.dataset_test))
            audits = []
            ...
            with torch.no_grad(), gzip.open(part, 'wt', compresslevel=3) as stream:
                for step, index in enumerate(indices):
                    ...
                    for case in CASES:
                        ...
                        if args.smoke and case in ['c', 'l', 'r', 'cl', 'cr', 'lr', 'clr']:
                        ...
                        del output
                    if args.smoke:
                        current = anno([line for line in record['pred']['clr'] if line.split()[0] != 'dummy' and float(line.split()[-1]) > 0.3])
                        original_lines = (REFERENCE / 'preds' / f'{index:06d}.txt').read_text().splitlines()
                        archived = anno([line for line in original_lines if line.split()[0] != 'dummy'])
                        for key in ['name', 'dimensions', 'location', 'rotation_y', 'score']:
                            assert current[key].shape == archived[key].shape, ('CLR archive shape', index, key, current[key].shape, archived[key].shape)
                            if key == 'name':
                                assert np.array_equal(current[key], archived[key]), ('CLR archive labels', index)
                            else:
                                assert np.allclose(current[key], archived[key], atol=1e-3, rtol=1e-4), ('CLR archive numerical drift', index, key, float(np.max(np.abs(current[key]-archived[key]))))
                        audits.append(dict(index=index, clr_archived_prediction_equivalent=True))
                    # Verify the old evaluation frame ordering and GT conversion.
                    old_gt = (REFERENCE / 'gts' / f'{index:06d}.txt').read_text().splitlines()
                    ...
                    if args.smoke or (step + 1) % 50 == 0:
                        ...
                        print(f'[{now()}] INFER {step+1}/{len(indices)} frames, nine new cases + CLR check/frame; {rate:.3f} frames/s, peak={torch.cuda.max_memory_allocated()/2**30:.2f} GiB', flush=True)
            ...
            evaluate(out, pack, manifest['is_validation_updated'], smoke=args.smoke)
        status(out, phase='complete', completed_at=now(), failure_count=0, traceback=None)
