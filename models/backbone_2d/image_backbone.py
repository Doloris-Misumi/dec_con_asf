...
from .swin_compat import swin_t, swin_s, swin_b
...
try:
    ...
    # torchvision < 0.13: no Weights enums, no Swin
    from torchvision.models import (
    ...
    ResNeXt50_32X4D_Weights = ResNeXt101_32X8D_Weights = None
    MobileNet_V3_Small_Weights = MobileNet_V3_Large_Weights = None

Swin_T_Weights = Swin_S_Weights = Swin_B_Weights = None
...
class ImageBackbone(nn.Module):
    ...
    SUPPORTED_MODELS = {
        ...
        # 'resnext101_64x4d': unavailable in torchvision < 0.13
        # Swin: unavailable in torchvision < 0.13
    ...
    def _create_backbone(self, pretrained):
        ...
        if weights_enum is not None and hasattr(weights_enum, 'DEFAULT'):
            ...
            # torchvision < 0.13: use pretrained kwarg
    ...
    def forward(self, x):
        ...
        if self.model_name.startswith('resnet') or self.model_name.startswith('resnext'):
            # ResNet NCHW format
            # Skip conv1+bn1+relu+maxpool (too shallow), start from layer1
            # Output: layer1@/4, layer2@/8, layer3@/16  (matching Swin pattern)
            x = self.backbone[:5](x)          # conv1..maxpool + layer1 → /4, 256ch
            list_feats.append(x)
            x = self.backbone[5](x)            # layer2 → /8, 512ch
            list_feats.append(x)
            x = self.backbone[6](x)            # layer3 → /16, 1024ch
            list_feats.append(x)
        else:
            # Swin NHWC format
            x = self.backbone[:4](x)
