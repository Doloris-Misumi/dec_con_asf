import os
from . import evaluation_common as kitti
from .kitti_official_evaluate import get_official_eval_result


def get_frame_list_from_folder(folder):
    return sorted([os.path.splitext(x)[0] for x in os.listdir(folder) if x.endswith('.txt')])
...
if __name__ == '__main__':
    ...
    evaluation = Evaluation(test_annotation_file=os.path.join('/mnt/8tssd/AdverseWeather/view_of_delft_PUBLIC/radar_5frames/testing/', 'label_2'))
    # Using the evaluate method, the model can be evaluated on the detection labels.
    val_image_ids = get_frame_list_from_folder(os.path.join('/home/hx/OpenPCDet-master/output/VoD_models/PP_radar/normed/eval/epoch_80/val/default/final_result', 'data'))
    print(len(val_image_ids))
    results = evaluation.evaluate(
        result_path=os.path.join('/home/hx/OpenPCDet-master/output/VoD_models/PP_radar/normed/eval/epoch_80/val/default/final_result', 'data'),
        current_class=[0, 1, 2])
