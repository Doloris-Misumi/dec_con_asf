...
class KRadarFusion_v1_0(Dataset):
    def __init__(self, cfg=None, split='all'):
        ...
        if self.cam_process is not None:
            ...
            if self.img_load == 2:
                # Original images need resize to 256x704 for LSS
                self.compose_img = torchvision.transforms.Compose(
                    [
                        torchvision.transforms.ToTensor(),
                        torchvision.transforms.Resize((256, 704)),
    ...
    def get_camera_img(self, dict_item):
        if self.img_load == 0: # cropped
            ...
            if self.cam.front0 or self.cam.front1:
                ...
                dict_item['front0'] = self.compose_img(img_front[:,:1280,:])
                dict_item['front1'] = self.compose_img(img_front[:,1280:,:])
            if self.cam.left0 or self.cam.left1:
                img_front = cv2.imread(dict_path['left'])
                dict_item['left0'] = self.compose_img(img_front[:,:1280,:])
                dict_item['left1'] = self.compose_img(img_front[:,1280:,:])
            if self.cam.right0 or self.cam.left1:
                img_front = cv2.imread(dict_path['right'])
                dict_item['right0'] = self.compose_img(img_front[:,:1280,:])
                dict_item['right1'] = self.compose_img(img_front[:,1280:,:])
            if self.cam.rear0 or self.cam.rear1:
                img_front = cv2.imread(dict_path['rear'])
                dict_item['rear0'] = self.compose_img(img_front[:,:1280,:])
                dict_item['rear1'] = self.compose_img(img_front[:,1280:,:])
    ...
    def collate_fn(self, list_batch):
        ...
        if self.collate_ver == 'v2_0': # gt_boxes (B, M, 8)
            ...
            # Camera intrinsics handling - skip if camera not loaded
            if self.item.get('cam', False) and 'camera_intrinsics' in dict_batch:
                if isinstance(dict_batch.get('camera_intrinsics'), torch.Tensor):
                    dict_batch['sensor2image'] = torch.zeros((batch_size,1,4,4), dtype=torch.float32)
                    for idx_b in range(dict_batch['batch_size']):
                        shaped_intrinsic = dict_batch['camera_intrinsics'][idx_b,:,:,:]
                        ...
                        sen2cam0 = torch.inverse(dict_batch['camera2'+self.ref_sensor][idx_b,0,:,:])
...
if __name__ == '__main__':
    kradar_detection = KRadarFusion_v1_0()
