...
class FusionBaseIntegrated(nn.Module):
    ...
    def load_each_encoder(self, encoder_cfg, type='cam'):
        ...
        if encoder_cfg.PRETRAINED is not None:
            encoder.load_state_dict(torch.load(encoder_cfg.PRETRAINED), strict=False)
    ...
    def loss(self, batch_dict):
        ...
        if 'patch_dec_loss' in batch_dict:
            patch_dec_weight = self.model_cfg.LOSS.get('PATCH_DEC_WEIGHT', 0.1)
            patch_dec_loss = batch_dict['patch_dec_loss']
            patch_dec_loss_weighted = patch_dec_weight * patch_dec_loss
            loss += patch_dec_loss_weighted
            if self.is_logging:
                if 'logging' not in batch_dict:
                    batch_dict['logging'] = dict()
                batch_dict['logging'].update(batch_dict.get('patch_dec_logging', {}))
                batch_dict['logging'].update({
                    'patch_dec_loss_weighted': patch_dec_loss_weighted.item(),
                })
