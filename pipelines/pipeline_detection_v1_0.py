...
class PipelineDetection_v1_0():
    ...
    def build_dataloader(self, dataset, batch_size, shuffle, collate_fn, drop_last=False):
        num_workers = int(self.cfg.OPTIMIZER.get('NUM_WORKERS', 0))
        kwargs = dict(
        ...
        if self.cfg.OPTIMIZER.get('PIN_MEMORY', False):
            kwargs['pin_memory'] = True

        if num_workers > 0:
            if self.cfg.OPTIMIZER.get('PERSISTENT_WORKERS', False):
                kwargs['persistent_workers'] = True
            prefetch_factor = self.cfg.OPTIMIZER.get('PREFETCH_FACTOR', None)
    ...
    def train_network(self, is_shuffle=True):
        self.network.train()
        data_loader_train = self.build_dataloader(
            self.dataset_train,
    ...
    def validate_kitti(self, epoch=None, list_conf_thr=None, is_subset=False):
        ...
        for idx_datum, dict_datum in enumerate(data_loader):
            ...
            try:
                ...
                print('* Meta info: ', dict_datum.get('meta', None))
