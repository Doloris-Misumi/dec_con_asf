...
class CamBase(nn.Module):
    ...
    def _select_lss_feature(self, image_fpn):
        if isinstance(image_fpn, torch.Tensor):
            return image_fpn

        target_size = tuple(self.feature_size)
        for feat in image_fpn:
            if tuple(feat.shape[-2:]) == target_size:
                return feat

        avail_sizes = [tuple(feat.shape[-2:]) for feat in image_fpn]
        raise RuntimeError(
            f"No FPN feature map matches LSS FEATURE_SIZE {target_size}; "
            f"available sizes: {avail_sizes}"
        )
    ...
    def forward(self, batch_dict):
        ...
        x = self._select_lss_feature(batch_dict['image_fpn'])
        ...
        if self.accelerate and self.cache is not None:
            ...
            lidar2image = batch_dict['sensor2image'][:, 0:1, :, :].cuda()  # front camera only, matches front_img
            # print(lidar2image)
    ...
    def loss(self, batch_dict):
        loss = 0.
        depth_loss = torch.tensor(0.0, device=loss.device if isinstance(loss, torch.Tensor) else 'cpu')
        
        rpn_loss = self.head.loss(batch_dict)
        loss += rpn_loss

        ### depth loss ###
        if self.w_depth > 0 and 'depth_labels' in batch_dict:
            depth = batch_dict['depth_pred']
            depth_label = batch_dict['depth_labels'].cuda()
            if isinstance(depth_label, list):
                depth_label = torch.stack(depth_label)
