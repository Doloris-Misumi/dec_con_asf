    ...
    def _apply_patch_dec(self, keys, tokens, batch_dict):
        ...
        if self.training and len(tokens) >= 2:
            ...
            if idx is not None and idx.numel() > 0:
                common_sel = [tok.index_select(0, idx).float() for tok in common_tokens]
                unique_sel = [tok.index_select(0, idx).float() for tok in unique_tokens]
...
class TaskAwareDecControlledA2Fusion(DecControlledA2Fusion):
    ...
    def _class_loss(self, class_logits, class_targets, idx, fg_mask=None):
        ...
        target = target[valid]
        logits = class_logits.index_select(0, idx)[valid].float()
        counts = torch.bincount(target, minlength=self.dec_control_num_classes).to(
            device=logits.device,
            dtype=torch.float32,
        )
        weights = torch.ones(self.dec_control_num_classes, dtype=torch.float32, device=logits.device)
        present = counts > 0
